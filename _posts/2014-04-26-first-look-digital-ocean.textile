...
Even though I have worked a great deal with OpenStack and various virtualization technologies, I haven't really used infrastructure as a service all that much. Being Canadian has made using Amazon Web Services unlikely as I normally work at organizations that have concerns (justified or not) about their data staying in Canada. Usually that means AWS is off the table from the start. I've used Rackspace a bit to test out their Rackspace files product which is based on OpenStack Swift and I've also used Heroku to deploy a "hubot":https://hubot.github.com/, but that's about it.
...
I mention the word "simplified" above. This is another way that I believe DO is distinguishing itself from other IaaS providers--a simplified web interface, and possibly a simplified product offering, and easy to understand pricing.
...
Now it's active again. Let's try ssh one more time...
