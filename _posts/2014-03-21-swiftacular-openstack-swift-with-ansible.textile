...
Most people who use the Internet or smartphones with mobile apps use object storage, they just don't know it because it's used in the background by the applications, such as ones that need to store many, many files, such as pictures. 
...
The idea is that you could do some quality of service on the replication network, or split it off entirely from the standard Swift network, as likely the replication network would go across the link between regions, which are likely going to be in different data centers. The link would likely be lower bandwidth and higher latency. 
...
As an example, when a storage server is configured we can tell the proxy to add it's devices to the ring.
...
Delegation is very handy, especially with OpenStack Swift.

h2. SSL/TLS

I don't know why but much of the OpenStack documentation, especially around Keystone, which is the authentication system, avoids discussing how to deploy SSL enabled services. 

Swiftacular sets up Keystone and the swift-proxy server with SSL. Certainly it's not setup as you would do it in production (likely with with a dedicated set of SSL termination servers) but it's a good example. I don't think the OpenStack documentation should show deploying systems such as Keystone without SSL.

h2. Conclusion

Using the Swiftacular Ansible playbooks and roles, and using the provided Vagrantfile, it's fairly easy to get a small OpenStack cluster going on a good laptop. It also includes regions and a replication network setup. This could be a good basis for starting out with OpenStack Swift. 
