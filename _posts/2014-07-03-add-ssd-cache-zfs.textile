...
I just physically added a ssd into my "home backup server":http://localhost:4000/2014/07/01/zfs-ubuntu-trusty.html and I would like to configure it as a zfs l2arc cache device. Doing so will mean recently used files will be accessed via the ssd drive instead of the slower spinning disks.
...
We can run some fio tests on it. Interestingly zfs doesn't support "direct=1". Note that this is just a basic, example fio test. Fio is my favorite disk performance too.
