...
To use docker on OSX it has to be done inside a Linux virtual machine running in a hypervisor (like Virtualbox or VMWare Fusion) on top of OSX, likely via Vagrant.
...
The vms are getting IPs from dnsmasq which is configured by default by qemu.
