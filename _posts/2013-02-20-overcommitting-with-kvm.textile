...
!https://raw.github.com/curtisgithub/curtisgithub.github.com/master/img/winter_cs.jpg!
_(Still winter in Edmonton, will have to blog a lot more in the summer)_
...
To me over committing means having virtual machines running on a node where to total CPU, main memory (ie. ram), and disk that is available to the virtual machines is much larger than the actual physical resources available on the node. Hopefully five or ten times larger.
...
This is also how OpenStack handles images. It puts one base image on the compute node and each s instance based on that same image is backed by a qcow2 snapshot. I suppose we could do the same with LVM, if desired, using snapshots.
