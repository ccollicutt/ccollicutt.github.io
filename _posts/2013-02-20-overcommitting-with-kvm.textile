...
To me over committing means having virtual machines running on a node where to total CPU, main memory (ie. ram), and disk than is available to the virtual machines is much larger than the actual physical resources available on the node. Hopefully five or ten times larger.
