...
In my situation I have various media files from short films I've made that I need to backup and protect from bitrot. To do that I've decided to use ZFS on Linux. I worked with ZFS + FreeBSD a bit, but I also want the ability to mount many different types of file systems, and surprisingly FreeBSD doesn't support that many of them. I'm also a big far of XFS, which I believe FreeBSD only supports in read-only mode. So Linux it is. 

h2. ZoL PPA

The easiest way to get ZoL is to use the ZFS-native PPA.

<pre>
<code>curtis@storage:~$ sudo apt-add-repository ppa:zfs-native/stable
</code>
</pre>
...
<pre>
<code>curtis@storage:~$ sudo apt-get install -y ubuntu-zfs 
SNIP!
zfs.ko:
