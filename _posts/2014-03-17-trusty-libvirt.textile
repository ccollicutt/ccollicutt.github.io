---
layout: post
title: Boot Ubuntu Trusty with libvirt
categories: 
---

p(meta). 17 March - 2014 - Edmonton!

h1. {{ page.title }}

Then next Ubuntu long term service (LTS) operating system, "Trusty Tahr or version 14.04":https://wiki.ubuntu.com/TrustyTahr/ReleaseSchedule, is currently expected to be released on April 27th of 2014, about a month away from the writing of this post.

Below I'll take a quick look at how to deploy the cloud image prepared by Ubuntu with libvirt. The server running libvirt is Ubuntu 12.04, and is setup in a default manner, which means vms should get natted access to the Internet. We'll use libvirt + kvm on Ubuntu Precise to boot the Trusty Tahr image.

h2. Download the image

First we download the cloud image from Ubuntu's site. Thankfully Ubuntu provides ready-to-go images. Thanks Ubuntu!
...
root# qemu-img convert -O qcow2 trusty-server-cloudimg-amd64-disk1.img.dist trusty-server-cloudimg-amd64-disk1.img
...
I'm not showing my public key below just because it doesn't fit well on the page. But just paste your public ssh key in after the "-".
...
With that file created, we can now boot the Trusty Tahr image.

h2. Prepare a libvirt.xml file

Here is an example libvirt xml file, in this case called trusty1.xml.

<pre>
<code>
root@tpaz01:~/cloud-images# cat trusty1.xml 
<domain type='kvm'>
    <name>trusty1</name>
...
h2. Define and start the virtual machine
...
<pre>
<code>
root@tpaz01:~/cloud-images# cat /var/lib/libvirt/dnsmasq/default.leases 
