...
p(meta). 25 May - 2014 - Edmonton!

!https://raw.githubusercontent.com/curtisgithub/curtisgithub.github.com/master/img/shipyard_login.png!
...
<pre>
<code>
vagrant@host1:~$ sudo apt-get install -y docker.io
</code>
</pre>
...
bq. Shipyard gives you the ability to manage Docker resources including containers, images, hosts, and more all from a single management interface. Shipyard can manage multiple Docker hosts giving the flexibility to build redundant, highly available applications.

There are actually several similar systems being worked on right now that can tie together multiple docker hosts. One example is Flynn which has "flynn-host":https://github.com/flynn/flynn-host, another is OpenStack which can manage docker containers, yet another is "CoreOS":https://coreos.com/, and there are more out there. Typically these systems will do much more than just manage containers (or jobs) but they will all have that functionality. (Feel free to correct me if I'm wrong.)

h2. Setup the shipyard host

This is as easy as starting a container. The shipyard project has a pre-created and distributed shiyard docker container. All we have to do is install that image and run it. Note that this will download a few hundred megabytes of images if they do not already exist on the docker host.

<pre>
<code>
vagrant@host1:~$ sudo docker.io run -i -t -v /var/run/docker.sock:/docker.sock shipyard/deploy setup
SNIP!
...
<pre>
<code>
vagrant@host1:/etc/default$ netstat -ant  |grep 4243
...
<pre>
<code>
vagrant@host1$ sudo ./shipyard-agent -url http://192.168.5.89:8000 -register
...
<pre>
<code>
vagrant@host1:/usr/local/bin$ sudo ./shipyard-agent -address="192.168.5.89" \
...
!https://raw.githubusercontent.com/curtisgithub/curtisgithub.github.com/master/img/shipyard_new_host.png!
...
!https://raw.githubusercontent.com/curtisgithub/curtisgithub.github.com/master/img/shipyard_hosts.png!
...
!https://raw.githubusercontent.com/curtisgithub/curtisgithub.github.com/master/img/shipyard_both_hosts.png!
...
!https://raw.githubusercontent.com/curtisgithub/curtisgithub.github.com/master/img/shipyard_busybox.png!


<pre>
<code>
vagrant@host1:/var/log/upstart$ sudo docker.io images | grep busybox
...
<pre>
<code>
vagrant@host2:~$ sudo docker.io images  |grep busybox
busybox     buildroot-2013.08.1   123fb16d32f8        26 hours ago        2.489 MB
busybox     ubuntu-14.04          b9ca777960b9        26 hours ago        5.609 MB
busybox     ubuntu-12.04          8ba0d1860bb6        26 hours ago        2.433 MB
busybox     buildroot-2014.02     4c0f792ebd1e        38 hours ago        2.433 MB
...
<pre>
<code>
curtis$ git clone https://github.com/shipyard/shipyard-cli
...
In the end what we have done here is fairly basic--just install a shipyard host and a couple of clients. I think this represents a good, simple implementation of that idea. Certainly there will be and are other systems that do the same thing and much more, but I think shipyard is a good way to get introduced to the concepts of a multihost docker system. I'd also like to automate the deployment of shipyard, but have a couple things to figure out, such as how to register the agent automatically, activating the hosts, as well as setup some sort of supervisory system for the agent. Also there are a few things that shipyard can do that I haven't touched on, such as the concept of applications.
