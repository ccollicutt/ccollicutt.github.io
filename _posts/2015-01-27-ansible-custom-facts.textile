...
I just wanted a simple script to parse the ip address of the server nad return a private ip based on the last octet of the address.
...
Using ansible I put that file into the /etc/ansible/facts.d directory. Note it has to be exectuable, return json, and end with the .fact extension.
