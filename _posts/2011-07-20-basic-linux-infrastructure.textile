...
There should be a central syslog server somewhere in your infrastucture, and all production servers should send syslog packages to that server. 
...
The kickstart server is simply a plain http server that serves kickstart files, and in conjunction with the packaging server allows for rapid, repeatable installation of Redhat/CentOS. Or you could do it over nfs, or pop it on the USB key. For me, I always use a web server to serve up the kickstarts.

@ks=http://example.com/ks/newserver.ks@ if you know what I mean. :)

*6. Centralize management*

This is a central server that can be used to access all the Linux/Unix servers (via ssh) and try to automate with custom scripts or things like pdsh, chef, puppet, ect.
...
Every IT workplace should have a central code repository. Sure, hg and git are distributed, but I still think it's nice to have a central location. "Github":github.com seems to be successful at centralizing distributed revision control, so I think it'll work for most workplaces as well. :)
