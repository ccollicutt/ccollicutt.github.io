...
For the most part it's used for getting used to how "Swift":http://docs.openstack.org/developer/swift/ works and is deployed and will usually be deployed using Vagrant and Virtualbox. One of the features I hope to add is the ability to use multiple provisioners, such as IaaS providers like as Digital Ocean, OpenStack, etc. Having said that, it's not required to use Vagrant, you could easily change some IPs around in the ansible hosts file and deploy to any servers, whethery they are provided by Vagrant, virtual servers or even bare metal.
